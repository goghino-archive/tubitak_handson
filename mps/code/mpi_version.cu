               ...
               float *solution, float *bestFitness_o, int *genNumber_o, double *time_o)
{
    ...
    //Hardcoded device no., we want to share device with id=2 (Kepler arch.)
    //amongst multiple MPI processes to see the performance influence
    //cudaSetDevice(2); //-- default GPU will be selected
